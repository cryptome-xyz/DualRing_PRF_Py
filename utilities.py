...
def hash_commit(salt, e, k, i, sd):
    string = str(salt) + str(e) + str(k) + str(i) + str(sd)
    return sha3_256_func(string)


def list_to_string(input):
    if not isinstance(input, list):
        return str(input)
    return ''.join(list_to_string(item) for item in input)
...
def compute_merkle_root(data, randomness):
    ...
    tree = {0: [sha3_256_func(str(data[i]) + str(rand_list[i])) for i in range(len(data))]}
    level = 0
    while len(tree[level]) > 1:
        tree[level + 1] = []
        for i in range(0, len(tree[level]), 2):
            node = sha3_256_func(str(tree[level][i]) + str(tree[level][i + 1]))
...
def recompute_MT_root(leaf, randomness, auth_path, index, length):
    # expand randomness to a rand_list with the same length as data
    rand = shake_128_func(randomness, length * 16)
    rand_list = []
    for i in range(length):
        rand_list.append(rand[i * 16: (i + 1) * 16])
    rand = rand_list[index]

    current_hash = sha3_256_func(str(leaf) + str(rand))
    for sibling_hash in auth_path:
        if index % 2 == 0:
            current_hash = sha3_256_func(str(current_hash) + str(sibling_hash))
        else:
            current_hash = sha3_256_func(str(sibling_hash) + str(current_hash))
        index = index // 2
